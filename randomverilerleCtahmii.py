...
# Hava Durumu Tahmini - LSTM Modeli
# Seattle hava durumu verisiyle maksimum sıcaklık tahmini yapıyoruz
...
for col in ['temp_max', 'precipitation', 'temp_min', 'wind']:
...
# ============================================================
target = 'temp_max' # Tahmin etmek istediğimiz değişken: maksimum sıcaklık
...
y_raw = data[target].values   # Çıktı (maksimum sıcaklık)
...
model = Sequential([ # Modeli sırayla katmanlar ekleyerek kuruyoruz
    ...
    Dense(1)        # Çıkış katmanı: tek değer (maksimum sıcaklık)
])
...
plt.plot(y_actual, label='Gerçek Sıcaklık (°C)', linewidth=1.5) # Gerçek değerler
plt.plot(y_pred,   label='LSTM Tahmin (°C)', linewidth=1.5, linestyle='--') # Tahmin edilen değerler
plt.title("Maksimum Sıcaklık: Gerçek vs Tahmin") # Başlık
